    ...
    <header class="bg-white shadow-md sticky md:static top-0 z-50" :class="{ 'dark': darkMode }">
        ...
        <div class="max-w-7xl mx-auto px-4 py-3 sm:px-6 lg:px-8 flex justify-between items-center">
            ...
            <nav class="hidden md:flex md:space-x-2 md:text-sm lg:space-x-6 lg:text-base font-medium">
                <Link href="/" class="text-gray-500 hover:text-blue-600 px-2 py-1 rounded">
                {{ t('navigation.home') || 'Главная' }}</Link>
                <Link href="/services" class="text-gray-500 hover:text-blue-600 px-2 py-1 rounded">
                {{ t('navigation.services') || 'Услуги' }}
                </Link>
                <Link href="/doctors" class="text-gray-500 hover:text-blue-600 px-2 py-1 rounded">
                {{ t('navigation.doctors') || 'Врачи' }}
                </Link>
                <Link href="/promotions" class="text-gray-500 hover:text-blue-600 px-2 py-1 rounded">
                {{ t('navigation.promotions') || 'Акции' }}
                </Link>
                <Link href="/contacts" class="text-gray-500 hover:text-blue-600 px-2 py-1 rounded">
                {{ t('navigation.contacts') || 'Контакты' }}
                </Link>
                <Link href="/schedule" class="border border-blue-600 text-blue-600 px-3 py-1 rounded hover:bg-blue-50">
                {{ t('navigation.schedule') || 'Расписание' }}</Link>
            </nav>

            <div class="flex items-center md:space-x-2 lg:space-x-4">
                <!-- Поиск -->
                <Link href="/search" class="text-gray-500 hover:text-blue-600">
                ...
                <Link v-if="$page.props.auth.user" href="/dashboard"
                    class="md:text-sm lg:text-base ml-3 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-medium hidden sm:inline">
                {{ t('navigation.dashboard') || 'Личный кабинет' }}
                </Link>

                <!-- Вход и Регистрация -->
                <div v-else class="flex items-center md:space-x-2 lg:space-x-4">
                    <Link href="/login"
                        class="mx-3 md:text-sm lg:text-base text-gray-500 hover:text-blue-600 hidden sm:inline">
                    {{ t('navigation.login') || 'Вход' }}</Link>
                    <Link href="/register"
                        class="md:text-sm lg:text-base px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-medium hidden sm:inline">
        ...
        <div class="mobile-menu md:hidden bg-white dark:bg-gray-800" :class="{ 'open': mobileMenuOpen }">
            ...
            <div class="px-4 py-2 space-y-2">
                ...
                {{ t('navigation.home') || 'Главная' }}</Link>
                <Link href="/services"
                    class="block px-2 py-2 text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-gray-700 text-center rounded">
                {{ t('navigation.services') || 'Услуги' }}</Link>
                <Link href="/doctors"
                    class="block px-2 py-2 text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-gray-700 text-center rounded">
                {{ t('navigation.doctors') || 'Врачи' }}</Link>
                <Link href="/promotions"
                    class="block px-2 py-2 text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-gray-700 text-center rounded">
                {{ t('navigation.promotions') || 'Акции' }}</Link>
                <Link href="/contacts"
                    class="block px-2 py-2 text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-gray-700 text-center rounded">
                {{ t('navigation.contacts') || 'Контакты' }}</Link>
                <Link href="/schedule"
                    class="block px-2 py-2 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-gray-700 text-center rounded border border-blue-600 dark:border-blue-400">
                {{ t('navigation.schedule') || 'Расписание' }}</Link>
            </div>

            <!-- Личный кабинет -->
            <div v-if="$page.props.auth.user" class="px-4 py-3 border-t border-gray-200 dark:border-gray-700 sm:hidden">
                <Link href="/dashboard"
                    class="block w-full px-2 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded text-center">
                {{ t('navigation.dashboard') || 'Личный кабинет' }}</Link>
            </div>

            <!-- Вход и Регистрация -->
            <div v-else class="px-4 py-3 border-t border-gray-200 dark:border-gray-700 sm:hidden">
                <Link href="/login"
                    class="block w-full px-2 py-2 mb-2 text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-gray-700 rounded text-center">
                {{ t('navigation.login') || 'Вход' }}</Link>
                <Link href="/register"
                    class="block w-full px-2 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded text-center">
                {{ t('navigation.register') || 'Регистрация' }}</Link>
            </div>
...
:root {
  ...
  --color-gray-100: #f3f4f6;     /* bg-gray-100, dark:text-gray-100 */
  --color-gray-200: #e5e7eb;     /* border-gray-200 */
  --color-gray-300: #d1d5db;     /* text-gray-300, dark:hover:bg-gray-700 */
  --color-gray-500: #6b7280;     /* text-gray-500 */
  --color-gray-700: #374151;     /* text-gray-700, dark:border-gray-700 */
  --color-gray-800: #1f2937;     /* dark:bg-gray-800 */
  --color-gray-900: #111827;     /* text-gray-900 */
  --color-gray-900-50: rgba(17, 24, 39, 0.5); /* dark:bg-gray-900/50 */

  --color-blue-50: #eff6ff;      /* bg-blue-50 */
  --color-blue-400: #60a5fa;     /* dark:text-blue-400 */
  --color-blue-600: #2563eb;     /* text-blue-600, border-blue-600 */
  --color-blue-700: #1d4ed8;     /* hover:bg-blue-700 */

  --color-white: #ffffff;        /* bg-white */
  --color-black: #000000;        /* (резерв) */
}
</style> -->